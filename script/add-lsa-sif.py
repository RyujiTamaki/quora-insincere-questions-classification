...
num_topics = 50 # number of topics of LSA feature
...
class NeuralNet(nn.Module):
    def __init__(self,
        ...
        self.linear = nn.Linear(lstm_hidden_size * 8 + num_topics + embed_size, last_hidden_size)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout_rate)
        self.out = nn.Linear(last_hidden_size, 1)

    def forward(self, x_tokens, x_lsa, x_sif):
        ...
        conc = torch.cat((h_lstm_atten, h_gru_atten, max_pool, avg_pool, x_lsa, x_sif), 1)
...
# LSA
###
texts = [text.split() for text in np.concatenate([x_train, x_test])]
dictionary = corpora.Dictionary(texts)
corpus = [dictionary.doc2bow(text) for text in texts]
tfidf_model = gensim.models.TfidfModel(corpus=corpus)
tfidf_corpus = tfidf_model[corpus]
tfidf_mat = gensim.matutils.corpus2csc(tfidf_corpus).T
x_lsa = TruncatedSVD(n_components=num_topics, algorithm='arpack').fit_transform(tfidf_mat)
x_lsa_train = x_lsa[:train_df.shape[0]]
x_lsa_test = x_lsa[train_df.shape[0]:]
...
x_test_lsa = torch.tensor(x_lsa_test, dtype=torch.float).cuda()
x_test_sif = torch.tensor(x_sif_test, dtype=torch.float).cuda()
test = torch.utils.data.TensorDataset(x_test_cuda, x_test_lsa, x_test_sif)
test_loader = torch.utils.data.DataLoader(test, batch_size=2048, shuffle=False)
...
for i, (train_idx, valid_idx) in enumerate(splits):
    ...
    x_lsa_train_fold = torch.tensor(x_lsa_train[train_idx], dtype=torch.float).cuda()
    x_lsa_val_fold = torch.tensor(x_lsa_train[valid_idx], dtype=torch.float).cuda()
    ...
    train = torch.utils.data.TensorDataset(x_train_fold, x_lsa_train_fold, x_sif_train_fold, y_train_fold)
    valid = torch.utils.data.TensorDataset(x_val_fold, x_lsa_val_fold, x_sif_val_fold, y_val_fold)

    train_loader = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=True)
    valid_loader = torch.utils.data.DataLoader(valid, batch_size=2048, shuffle=False)
    ...
    for epoch in range(n_epochs):
        ...
        model.train()
        avg_loss = 0.
        for x_batch, x_lsa_batch, x_sif_batch, y_batch in train_loader:
            y_pred = model(x_batch, x_lsa_batch, x_sif_batch)
            scheduler.step()
        ...
        avg_val_loss = 0.
        for i, (x_batch, x_lsa_batch, x_sif_batch, y_batch) in enumerate(valid_loader):
            y_pred = model(x_batch, x_lsa_batch, x_sif_batch).detach()
            y_proba = sigmoid(y_pred.cpu().numpy())[:, 0]

            avg_val_loss += loss_fn(y_pred, y_batch).item() / len(valid_idx)
            valid_preds_fold[i * 2048:(i + 1) * 2048] = y_proba

        search_result = threshold_search(y_train[valid_idx], valid_preds_fold)
        elapsed_time = time.time() - start_time
        print('Epoch {}/{} \t loss={:.4f} \t val_loss={:.4f} \t f1={:.4f} \t time={:.2f}s'.format(
            epoch + 1, n_epochs, avg_loss, avg_val_loss, search_result['f1'], elapsed_time))
        ...
        if avg_val_loss < min_val_loss:
            ...
            print('did not improve from {:.4f}'.format(min_val_loss))

    for i, (x_batch, x_lsa_batch, x_sif_batch) in enumerate(test_loader):
        model = NeuralNet()
        ...
        test_preds_fold[i * 2048:(i + 1) * 2048] = sigmoid(y_pred.cpu().numpy())[:, 0]
