...
maxlen = 35  # max number of words in a question to use
...
def train_model(model, x_train, y_train, x_val, y_val, i, validate=True):

    train = torch.utils.data.TensorDataset(x_train, y_train)
    valid = torch.utils.data.TensorDataset(x_val, y_val)

    train_loader = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=True)
    valid_loader = torch.utils.data.DataLoader(valid, batch_size=pred_batch_size, shuffle=False)
    ...
    for epoch in range(n_epochs):
        ...
        if validate:
            ...
            for i, (x_batch, y_batch) in enumerate(valid_loader):
                ...
                valid_preds[i * pred_batch_size:(i+1) * pred_batch_size] = sigmoid(y_pred.cpu().numpy())[:, 0]
    ...
    test_preds = np.zeros((len(test_pre_loader.dataset)))

    for test_loader in [test_pre_loader, test_post_loader]:
        for i, (x_batch,) in enumerate(test_loader):
            y_pred = model(x_batch).detach()

            test_preds[i * pred_batch_size:(i+1) * pred_batch_size] += sigmoid(y_pred.cpu().numpy())[:, 0] / 2

    test_preds_local = np.zeros((len(test_local_post_loader.dataset)))

    for test_local_loader in [test_local_pre_loader, test_local_post_loader]:
        for i, (x_batch,) in enumerate(test_local_loader):
            y_pred = model(x_batch).detach()
            test_preds_local[i * pred_batch_size:(i+1) * pred_batch_size] += sigmoid(y_pred.cpu().numpy())[:, 0] / 2
...
x_test_pre = pad_sequences(x_test, maxlen=maxlen, truncating='pre')
x_test_local_pre = pad_sequences(x_test_local, maxlen=maxlen, truncating='pre')

x_train_post = pad_sequences(x_train, maxlen=maxlen, truncating='post')
x_test_post = pad_sequences(x_test, maxlen=maxlen, truncating='post')
x_test_local_post = pad_sequences(x_test_local, maxlen=maxlen, truncating='post')
...
splits = list(StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=10).split(x_train, y_train))
train_preds = np.zeros((len(train_df)))
test_preds = np.zeros((len(test_df)))

x_test_pre_cuda = torch.tensor(x_test_pre, dtype=torch.long).cuda()
test_pre = torch.utils.data.TensorDataset(x_test_pre_cuda)
test_pre_loader = torch.utils.data.DataLoader(test_pre, batch_size=pred_batch_size, shuffle=False)

x_test_local_pre_cuda = torch.tensor(x_test_local_pre, dtype=torch.long).cuda()
test_local_pre = torch.utils.data.TensorDataset(x_test_local_pre_cuda)
test_local_pre_loader = torch.utils.data.DataLoader(test_local_pre, batch_size=pred_batch_size, shuffle=False)

x_test_post_cuda = torch.tensor(x_test_post, dtype=torch.long).cuda()
test_post = torch.utils.data.TensorDataset(x_test_post_cuda)
test_post_loader = torch.utils.data.DataLoader(test_post, batch_size=pred_batch_size, shuffle=False)

x_test_local_post_cuda = torch.tensor(x_test_local_post, dtype=torch.long).cuda()
test_local_post = torch.utils.data.TensorDataset(x_test_local_post_cuda)
test_local_post_loader = torch.utils.data.DataLoader(test_local_post, batch_size=pred_batch_size, shuffle=False)
...
for i, (train_idx, valid_idx) in enumerate(splits):
    ...
    if i % 2 == 0:
        x_train_fold = torch.tensor(x_train_pre[train_idx], dtype=torch.long).cuda()
        y_train_fold = torch.tensor(y_train[train_idx, np.newaxis], dtype=torch.float32).cuda()
        x_val_fold = torch.tensor(x_train_pre[valid_idx], dtype=torch.long).cuda()
        y_val_fold = torch.tensor(y_train[valid_idx, np.newaxis], dtype=torch.float32).cuda()
    else:
        x_train_fold = torch.tensor(x_train_post[train_idx], dtype=torch.long).cuda()
        y_train_fold = torch.tensor(y_train[train_idx, np.newaxis], dtype=torch.float32).cuda()
        x_val_fold = torch.tensor(x_train_post[valid_idx], dtype=torch.long).cuda()
        y_val_fold = torch.tensor(y_train[valid_idx, np.newaxis], dtype=torch.float32).cuda()
...
if enable_local_test:
    ...
    search_result = threshold_search(y_test, test_preds_local.mean(axis=1))
